...
ha <- 
  read_csv(here("data", "ha_clean.csv"), col_types = cols(
...
PortoAlegre_1ha <-ha_1frag %>%
  ...
  dplyr::select(ranch,plot,habitat,ha_id_number,year,ht,row,column)
...
ha_data_filter <- function(data,ranch_name,habitat_type) {
  data %>%
    dplyr::filter(ranch == ranch_name & habitat == habitat_type) %>%
    dplyr::select(ranch, plot, habitat, ha_id_number, year, ht, row, column)
}
...
ha2 %>%
  dplyr::filter(plot=="2107") %>%
  dplyr::filter(ha_id_number=="100") %>%
  dplyr::select(ha_id_number,row,column)

ha2 %>%
  dplyr::filter(plot=="2107") %>%
  dplyr::filter(ha_id_number == "262") %>%
  dplyr::select(ha_id_number,row,column)
...
one_ha_coord_final<-
  ha4 %>% 
  mutate(x_final = plot_x + x) %>%
  mutate(y_final = plot_y + y)
# view(one_ha_coord_final)
