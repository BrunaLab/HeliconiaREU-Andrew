...
ggplot(data=dist_10m)+
  geom_histogram(aes(x=log_size,fill=bdffp_reserve_no))

ggplot(data=dist_fine)+
  geom_histogram(aes(x=distance_to_nearest_edge,fill=bdffp_reserve_no))
...
surv.fine.m4 <- glmer(surv~log_size_prev*bdffp_reserve_no*distance_to_nearest_edge+(1|year),
                      data=dist_fine,family=binomial, control=glmerControl(optimizer="nloptwrap"))

isSingular(surv.fine.m4)
...
nwdat.df <-broom::augment(surv.fine.m2, type.predict = "response", newdata = newdata,
  ...
  geom_ribbon(aes(ymin=.fitted-1.405*.se.fit,ymax=.fitted+1.405*.se.fit),fill="black",alpha=0.5)+
...
flwr.m3 <- glmer(flwr~log_size_prev*dist_near*bdffp_reserve_no+flwr_prev+(1|year),
                 data=dist_10m,family=binomial,control=glmerControl(optimizer="bobyqa"))
...
flwr.factor <-data.frame(flwr_prev=rep(1,501))
flwr.factor$flwr_prev <- as.factor(flwr.factor$flwr_prev)
...
geom_hline(aes(yintercept=CF.flwr.df$.fitted,color="CF"))+
  geom_ribbon(aes(ymin=CF.flwr.df$.fitted-1.405*CF.flwr.df$.se.fit,
              ...
              fill="red",alpha=0.5)+
  labs(title="Comparison of flowering probability for upper quantile sized plants in FF and CF", subtitle= "Plants did flower previous year",
       x = "Distance to nearest edge", y= "Predicted probability of flowering")

flwr.factor <- data.frame(flwr_prev=rep(0,501))
flwr.factor$flwr_prev <- as.factor(flwr.factor$flwr_prev)
...
geom_hline(aes(yintercept=CF.flwr.df$.fitted,color="CF"))+
  geom_ribbon(aes(ymin=CF.flwr.df$.fitted-1.405*CF.flwr.df$.se.fit,
                  ymax=CF.flwr.df$.fitted+1.405*CF.flwr.df$.se.fit),
              fill="red",alpha=0.5)+
  labs(title="Comparison of flowering probability for upper quantile sized plants in FF and CF", subtitle="Plants did not flower previous year",
       x = "Distance to nearest edge", y= "Predicted probability of flowering")
...
check_outliers(log_size.m4)
performance::check_model(log_size.m4)
tab_model(log_size.m4)
```
...
check_outliers(log_size_fine.m4)
performance::check_model(log_size_fine.m4)
tab_model(log_size_fine.m4)
